...
class WidgetSimpleScan(DefaultScanWidget):

    def __init__ (self, mainwin, parent = None):
        super(WidgetSimpleScan, self).__init__(mainwin, parent)
        self.setWindowTitle('Simple Scan')
        self.populateForm()
...
#        self.mainwin.client.sendLineAndGetAnswer('GET SICMINFO', self._populateFormCB)
    ...
    def _populateFormCB(self, config):
        ...
        self.addColumn()
        self.addLineEdit('StartPos', 'Vertical start position of the scan in micm (int)')
    ...
    def sendSettings(self):
        ...
        client.sendLine('SET mode=simpleScan')
        for setting, field in self.settings.iteritems():
            client.sendLine('SET simpleScan.'+setting+'='+str(field.text()))
    def _prepare(self):
